...
use aidoku::{
    Chapter, ContentRating, FilterValue, ImageRequestProvider,
...
impl EpsilonSoft {
            ...
            if let Some(items) = html.select(selector) {
                for item in items {
                    ...
                    if let Some(link_elem) = link {
                        ...
                        let chapter_url = make_absolute_url(BASE_URL, &href);
    ...
    fn parse_page_list(&self, html: Document) -> Result<Vec<Page>> {
        ...
        // Madara's standard selectors including page-break, blocks-gallery-item, and reading-content
        let selectors = [
            "div.page-break img",
