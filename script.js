...
async function loadSources() {
    ...
    try {
        ...
        sortedSources.forEach((source, index) => {
            const sourceCard = createSourceCard(source, offlineSources, sourceTypes, index);
        ...
        sourcesContainer.innerHTML = '<div class="loading">Failed to load sources. Please try refreshing the page.</div>';
    }
}
...
function createSourceCard(source, offlineSources, sourceTypes, index) {
    ...
    card.innerHTML = `
        <div class="source-header">
            <img src="${iconPath}" alt="${source.name}" class="source-icon" onerror="this.style.display='none'">
        ...
        <div class="source-status">
            ...
            ${source.nsfw === 1 ? '<span class="badge badge-nsfw">NSFW</span>' : ''}
        </div>
...
function updateStats(sources, offlineSources) {
    ...
    const activeSources = sources.filter(source => !offlineSources.includes(source.id)).length;
    const offlineCount = sources.filter(source => offlineSources.includes(source.id)).length;
    
    // Update the description paragraph
    const guideSection = document.querySelector('.guide-section > p:first-child');
    if (guideSection) {
        guideSection.textContent = `${totalSources} French sources for Aidoku 0.8 (${activeSources} active, ${offlineCount} offline)`;
    }
